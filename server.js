...
app.get("/get_all_payments_mership", async (req,res)=>{
    ...
    const sql = "SELECT orders.Name_on_Parcel, (SELECT  IFNULL(users.Image, '') AS Image FROM users WHERE users.Id = payments.idReshipper) AS ImageReshipper, (SELECT  IFNULL(users.Image, '') AS Image FROM users WHERE users.Id = payments.idUser) AS ImageCustomar, users.Country, users.AddressLine1, CONCAT(users.First_Name,' ',users.Last_Name) AS Receiver_name, (SELECT CONCAT(users.First_Name,' ',users.Last_Name) FROM users WHERE users.Id = payments.idUser)AS SenderName , orders.Quantity,payments.datePayment,payments.paymentPrice, payments.priceAfterFees FROM `payments` JOIN orders ON payments.idOrder = orders.idOrder JOIN users ON payments.idReshipper = users.Id WHERE payments.idUser = ? OR payments.idReshipper = ? ;"
